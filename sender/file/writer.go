...
func newWriterStore(size int, interval time.Duration) *writerStore {
    if size <= 0 {
...
func (s *writerStore) NewWriter(filename string) (w *writer, _ error) {
    ...
    if err != nil {
        return nil, fmt.Errorf("open wc: %v", err)
    }
    w = &writer{
        wc: f,
    }
    if s.interval > 0 {
        iw, err := newIntervalWriter(f, s.interval)
        if err != nil {
            return nil, fmt.Errorf("create interval : %v", err)
        }
        w.wc = iw
    }
    ...
    for i := 1; i <= len(s.writers)-s.size; i++ {
        ...
        if len(expiredFilename) > 0 {
            if err := s.writers[expiredFilename].Close(); err != nil {
                log.Errorf("Failed to close expired wc writer %q: %v", expiredFilename, err)
            }
