...
func NewSender(conf conf.MapConf) (kafkaSender sender.Sender, err error) {
    ...
    sasl, _ := conf.GetBoolOr(KeyKafkaSASL, false)
    username, _ := conf.GetStringOr(KeyKafkaUserName, "")
    password, _ := conf.GetStringOr(KeyKafkaPassword, "")
    ...
    if sasl && len(username) > 0 && len(password) > 0 {
