...
func newSender(name, pattern, timestampKey string, maxOpenFile int, marshalFunc func([]Data) ([]byte, error), interval time.Duration) (*Sender, error) {
    ...
    return &Sender{
        ...
        writers:      newWriterStore(maxOpenFile, interval),
    }, nil
}
...
func NewSender(conf conf.MapConf) (sender.Sender, error) {
    ...
    interval, _ := conf.GetIntOr(KeyFileWriteInterval, 0)
    if rawMarshal {
        marshal = writeRawFunc
    }
    s, err := newSender(name, path, timestampKey, maxOpenFile, marshal, time.Duration(interval)*time.Millisecond)
    if err != nil {
        return nil, err
    }
...
func (s *Sender) writeFile(filename string, datas []Data, wg *sync.WaitGroup) {
    ...
    if err != nil {
        log.Errorf("get datas bytes to wc[%s] failed: %v, datas length: %d", filename, err, len(datas))
        return
    }
    _, err = s.writers.Write(filename, datasBytes)
    if err != nil {
        log.Errorf("write to wc[%s] failed: %v, datas length: %d", filename, err, len(datas))
